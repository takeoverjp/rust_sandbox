...
fn bm1(c: &mut Criterion) {
    c.bench_function("Pass String as reference", |b| {
        b.iter(|| ref_string(Some("test string".to_string())))
    });
}

fn bm2(c: &mut Criterion) {
    c.bench_function("Pass String with move", |b| {
        b.iter(|| move_string(Some("test string".to_string())))
    });
}
